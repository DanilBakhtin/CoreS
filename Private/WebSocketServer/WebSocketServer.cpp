...
#include <string>
#include "WebSocketNetworking/Public/IWebSocketServer.h"

DEFINE_LOG_CATEGORY_STATIC(LogAWebSocketServer, All, All);
...
bool AWebSocketServer::Start(int Port)
{
	...
	if (FModuleManager::Get().IsModuleLoaded("WebSocketNetworking"))
	{
		IWebSocketNetworkingModule& WebSocketModule = FModuleManager::Get().GetModuleChecked<IWebSocketNetworkingModule>("WebSocketNetworking");
		Server = WebSocketModule.CreateServer();
		UE_LOG(LogAWebSocketServer, Log, TEXT("WebSocketNetworking module is loaded!"));
	}
	else
	{
		FModuleManager::Get().LoadModule("WebSocketNetworking");
		UE_LOG(LogAWebSocketServer, Error, TEXT("WebSocketNetworking module is not loaded!"));
	}
...
void AWebSocketServer::ReceivedRawPacket(void* Data, int32 Size, FGuid ClientId)
{
	...
	OnJsonReceived.Broadcast(frameAsFString);
}
